...
TARGET_NAME := libretro_opk_core_make
LIBM		= -lm
...
OBJECTS := libretro_opk_core_make.o
OBJECTS += libopk.o
OBJECTS += unsqfs.o
OBJECTS += libini.o
...
$(TARGET): $(OBJECTS)
ifeq ($(STATIC_LINKING), 1)
	#/opt/gcw0-toolchain/usr/bin/$(AR) rcs $@ $(OBJECTS)
	$(AR) rcs $@ $(OBJECTS)
else
	#/opt/gcw0-toolchain/usr/bin/$(CC) $(fpic) $(SHARED) $(INCLUDES) -o $@ $(OBJECTS) $(LDFLAGS)
	$(CC) $(fpic) $(SHARED) $(INCLUDES) -o $@ $(OBJECTS) $(LDFLAGS)
endif

%.o: %.c 
	#/opt/gcw0-toolchain/usr/bin/$(CC) $(CFLAGS) $(fpic) -c -o $@ $<
	$(CC) $(CFLAGS) $(fpic) -c -o $@ $<
